    ...
    if (items.length) {
        ...
        items.forEach(item => {
            ...
            if ( !hasOwnProp.call(item, 'longname') ) {
                itemsNav += `<li>${getIcon(itemHeading)}<b>${linktoFn('', item.name)}</b></li>`;
            }
                ...
                itemsNav += [
                    `<li>`,
                    `${getIcon(itemHeading)}${hasMethods ? "<b>" : ""}${linktoFn(item.longname, displayName.replace(/\b(module|event):/g, ''))}${hasMethods ? "</b>" : ""}`,
                    hasMethods ?
                        [
        ...
        if (itemsNav !== '') {
            nav += `<h5 class="no-bottom must-underline">${itemHeading}</h5><ul>${itemsNav}</ul>`;
        }
    }
...
function getIcon (name) {
    ...
    return `<span class="small-icon ${icon}">${icon}</span>`;
}
...
function buildNav(members) {
    let globalNav;
    let nav = '<h4><a href="index.html">Home</a></h4>';
    const seen = {};
    const seenTutorials = {};
